...
class Usuario
{
    ...
    public function createRFID() {
        $query = "INSERT INTO RFID_TAGS (
        RFID_TAG_CODE,
        US_ID,
        RFID_STATUS,
        RFID_DATA_EMISSAO,
        ...
        '2E20F804',                        -- Código RFID
        LAST_INSERT_ID(),                  -- Pega o ID automaticamente
        'ATIVO',                           -- Status
        CURDATE(),                         -- Data emissão (hoje)
        DATE_ADD(CURDATE(), INTERVAL 1 YEAR) -- Expira em 1 ano
    );";
     $stmt = $this->conn->prepare($query);
      $stmt->execute();
    return $stmt;
    ...
    public function buscarPlanos($planoSelecionado = null)
    {
        $result = $this->conn->query("SELECT PL_ID, PL_NOME from Planos ORDER BY PL_NOME");
        $options = '';

        while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            $selected = ($planoSelecionado && $row['PL_ID'] == $planoSelecionado) ? 'selected' : '';
            $options .= "<option value='{$row['PL_ID']}' {$selected}>{$row['PL_NOME']}</option>";
        }
            return $options;
    }
